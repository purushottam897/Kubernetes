...
function Profile() {
  ...
  useEffect(() => {
    ...
    // Use manual data instead of fetching from database
    ...
    setUserDetails(manualUserDetails);
    setLoading(false);
  }, [authToken, navigate]);
  ...
  return (
    <div>
      ...
      <div className="profile-card" id="pbg" style={{ marginTop: '3%' }}>
        ...
        <h2 className="profile-name">{userDetails?.username || 'User'}</h2>
        ...
        <div>
          <h4>Phone Number: </h4>
          <p className="profile-phno">{userDetails?.phno || 'N/A'}</p>
        </div>
        <div>
        <h4>Gender: </h4>
        <p className="profile-gender">{userDetails?.gender || 'N/A'}</p>
      </div>
      <div>
        <h4>Date of Birth: </h4>
        <p className="profile-dob">{userDetails?.dob || 'N/A'}</p>
      </div>
      <div>
        <h4>Profession: </h4>
        <p className="profile-gender">{userDetails?.profession || 'N/A'}</p>
      </div>
        <div>
